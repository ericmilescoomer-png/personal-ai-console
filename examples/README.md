...
All examples below share one mock scenario (a "summarize the morning's meeting notes" mission) so the artifacts line up with each other.
...
| [`sample-agent-definition.json`](sample-agent-definition.json) | The shape of an agent definition: its owner-granted capability set (the whole of its authority) and its per-agent autonomy level (which controls confirmation frequency, not permission). See [docs/how-it-works.md](../docs/how-it-works.md). |
| [`sample-denial.json`](sample-denial.json) | A structured denial: what was blocked, why, and what would unlock it. A denial is the system working as designed, not an error. See [docs/trust-quartet.md](../docs/trust-quartet.md). |
