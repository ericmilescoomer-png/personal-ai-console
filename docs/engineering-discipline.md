...
The private build is covered by **nearly 2,200 automated tests** across 280-plus test files. They are organized into tiers, each answering a different question:
...
Nearly 400 contract tests, across more than sixty modules, exist for one reason: to make the trust model's promises break the build if they are ever violated. Among the invariants pinned this way:
