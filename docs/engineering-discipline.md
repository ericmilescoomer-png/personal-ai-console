...
The private build is covered by **2,395 automated tests** across **304 test files** (counts verified against the build on 2026-08-04) — including a **105-test acceptance suite** for the smart-home extension, property-based tests that gauntlet its signed receipt chain rather than merely exercise it. The suite is organized into tiers, each answering a different question:
...
**543 contract tests, across 95 modules**, exist for one reason: to make the trust model's promises break the build if they are ever violated. Among the invariants pinned this way:
