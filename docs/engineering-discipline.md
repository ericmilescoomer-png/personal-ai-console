...
The private build is covered by **more than 1,600 automated tests** across 230-plus test files. They are organized into tiers, each answering a different question:
...
More than 160 contract tests, across more than two dozen modules, exist for one reason: to make the trust model's promises break the build if they are ever violated. Among the invariants pinned this way:
