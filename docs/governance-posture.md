...
Personal A.I. Console&trade; (PAC) was not built to pass a compliance checklist. It was built by someone who believed that if AI is going to act on a person's behalf, there had to be a real system of controls around it: not a disclaimer, not a terms-of-service, not a trust-me.
...
It is written for someone who works in AI governance, AI risk, or AI compliance, and who wants to understand what this system implements, not just what it claims.
...
**The model never self-assigns its risk tier.** Tier assignment lives in PAC Core's capability registry, not in model output. A model that produces a persuasive argument for why a SENSITIVE action should be treated as SAFE does not move the tier; the registry is the source of truth, not the conversation.
...
AI governance frameworks (NIST AI RMF, EU AI Act, ISO/IEC 42001) all converge on a common requirement: consequential AI decisions must involve meaningful human oversight. The challenge is defining what "meaningful" means in a system that is supposed to be useful.

PAC implements human-in-the-loop as a structural property, not a UI choice:

- **Approval gates are pre-execution, not post-hoc.** Before a SENSITIVE step runs, the owner sees the full proposed plan, each step's risk tier, and what specifically requires confirmation. Approval is given before anything consequential happens, not surfaced after the fact as an alert.
...
- **Receipted system prompt changes.** The owner can edit the base system prompt that frames the model's behavior, but every change is versioned, receipted, and revertible. The framing history is evidence.
...
**Map**: *identify context, categorize risk, understand the AI's operating environment.*
...
- A dedicated contract-test layer pins the trust invariants specifically: 956 tests across 106 modules at the last verified count (see the README's *By the numbers*), so the coverage is against the governance properties, not just functional behavior.
...
The EU AI Act classifies AI systems by risk level and imposes requirements proportional to that risk. PAC is a personal, private-use system, not a commercial product or a high-risk application under the Act, but the design choices it makes track the Act's reasoning about what safe AI deployment looks like.
...
The short version: the two items marked fully addressed by core design are **Tool Misuse (ASI02)**, the capability tier registry, and **Rogue Agents (ASI10)**, the oversight split that prevents the delegate from silencing its watchers. Those are the system's hardest properties and the ones it was most deliberately designed around.
...
- Enterprise governance patterns (multi-user identity, ephemeral scoped tokens, formal SBOM, SOC 2 controls) are not part of the current single-owner local design.
