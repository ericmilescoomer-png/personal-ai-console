...
- A dedicated contract-test layer pins the trust invariants specifically &mdash; 956 tests across 106 modules at the last verified count (see the README's *By the numbers*) &mdash; so the coverage is against the governance properties, not just functional behavior.
